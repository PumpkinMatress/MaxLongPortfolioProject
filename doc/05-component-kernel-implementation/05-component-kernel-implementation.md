...
I plan to use three Queue components as representation,one Queue<String>, Queue<Integer>, and Queue<Double>. These parallel queues will respectively house print names, times, and filament costs. The representation will be restricted by the fact that all queues will have equal length, as the parallel nature of them requires all three fields to match up, and the available filament must be >=0.
